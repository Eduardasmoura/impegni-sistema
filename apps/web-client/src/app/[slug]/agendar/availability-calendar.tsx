...
// Mesma classificação pedida: 0 -> sem nenhum horário (indisponível, sem
// linha); 1-2 -> vermelho; 3-5 -> laranja; >5 -> verde.
    ...
    <div>
      ...
      <div className={cn("grid grid-cols-7 gap-1 sm:gap-1.5", loading && "opacity-50 pointer-events-none")}>
        {celulas.map((dia, i) => {
          ...
          // Continua clicável mesmo sem vaga (só não pode ser dia passado) —
          // é assim que o cliente chega no dia lotado e vê a opção de
          // entrar na lista de espera (Fase 5). Só o passado é bloqueado de
          // verdade; "sem vaga" muda a cor, não a possibilidade de clicar.
          ...
          return (
            ...
            >
              <span>{dia}</span>
              {/* Linha fina de disponibilidade — não pinta o fundo do dia inteiro. */}
